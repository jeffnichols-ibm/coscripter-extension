...
var Preferences = {
  DUMP_DEBUG: false,
  ERROR_LOG_DEBUG:false
}

var filename = 'coscripter-run-security-bypass-protocol.js';
function debug(msg) {
	if(Preferences.DUMP_DEBUG === true){
		dump(filename + " : " + msg + "\n");
	}
	if(Preferences.ERROR_LOG_DEBUG === true){
...
const kSCHEME = "coscriptrun";
const kPROTOCOL_NAME = "CoScripterRun Protocol";
const kPROTOCOL_CID = Components.ID("faeaffb2-3ec0-4172-9863-3503f30ef543");
...
Protocol.prototype =
{
  ...
  newChannel: function(aURI)
  {
    ...
    // first, we'll get a pointer to our friendly utility library (if we haven't already)
    // now we'll give a nice name to the url    
    var url = aURI.spec
    
    // ok, the url will look like "coscriptrun:http://coscripter.almaden.ibm.com/coscripter/lite/procedure/23",
    // and we want just the "http://coscripter.almaden.ibm.com/coscripter/lite/procedure/23" part...    
    if (url.match(/^[^:]+:(.*)/)) {
        // ...and now we have it
        ...
        mainChromeWindow.coscripter.loadProcedureIntoSidebar(procedureUrl, true)
    }
